...
\*\*Repo:\*\* https://github.com/ks2435/module12\_is601
...
\- `POST /calculations` — Add (computes result via factory; requires `user\_id`).
...
4\. `POST /calculations` with `{"a":10,"b":4,"type":"add","user\_id":<id>}`.
...
Required repo secrets: `DOCKERHUB\_USERNAME`, `DOCKERHUB\_TOKEN`.
...
\*\*What I built.\*\* I merged the User model from Module 10 and the Calculation model from Module 11 behind one FastAPI app. Passwords are hashed with bcrypt; the `/users/login` endpoint verifies them and returns the user record. Calculation endpoints implement full BREAD, with each calculation linked to a `user\_id`.
